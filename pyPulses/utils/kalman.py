"""
Implementation of a Kalman Filter compatible with the nonlinear EKF variety. A 
Kalman filter is intended to optimally converge on the true state by calculating
the optimal mixing weights of extrapolated states (there must be a model for the 
process) and new measurements. The extended Kalman filter (EKF) is not optimal 
due to the linearization, and it tends to underestimate covariance. It may also 
diverge if the measurement and process noise are not set appropriately. 
