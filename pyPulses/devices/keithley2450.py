...
@register_hardware_class("keithley2450")
class keithley2450(pyvisaDevice):
    """Class representation of the Keithley 2450."""
    ...
    def resolve(self, accessor: str) -> ScalarChannelAdapter:
        if accessor == 'V':
            return keithley2450_V_channel(self)
        if accessor == 'I':
            return keithley2450_I_channel(self)
        raise ValueError(f"keithley2450 Cannot resolve accessor: {accessor}")

class keithley2450_V_channel(ScalarChannelAdapter):
...
class keithley2450_I_channel(ScalarChannelAdapter):
    def __init__(self, parent: keithley2450):
        super().__init__(parent, 'V')
