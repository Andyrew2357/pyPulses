"""
Localhost web GUI for lock-in amplifiers.

Usage
-----
    gui = LockinGUI(my_sr860).start()  # opens at http://localhost:8760
    # ...
...
class LockinGUI:
    ...
    Wraps any object implementing the shared lock-in interface (see
    `lockin.base.LockinLike`) and launches a browser-based control panel
    ...
    Parameters
    ----------
    instrument : LockinLike
        Any object implementing the shared lock-in interface (SRS
        sr830/sr844/sr850/sr860/sr865a, AMETEK/Signal Recovery 7280, ...).
    ...
    def __init__(self, instrument: LockinLike, port: int = 8760, poll_interval: float = 0.5):
    ...
    def start(self) -> 'LockinGUI':
        ...
        self._thread = threading.Thread(
            target=self._run_asyncio, daemon=True, name='LockinGUI-ws'
        )
        self._thread.start()
        ...
        self._http_thread = threading.Thread(
            ...
            daemon=True, name='LockinGUI-http',
        )
        self._http_thread.start()

        self._instrument.gui = self
        print(f"Lock-in GUI → http://localhost:{self._port}")
        return self
    ...
    def _json_safe(obj):
        ...
        if isinstance(obj, dict):
            return {k: LockinGUI._json_safe(v) for k, v in obj.items()}
        if isinstance(obj, (list, tuple)):
            return [LockinGUI._json_safe(v) for v in obj]
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"LockinGUI({self._instrument.__class__.__name__}, "
